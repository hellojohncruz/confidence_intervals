...
Based on the sample, 65% of adults thinks climate change affects their
local community.
...
I would not expect another student's sample proportion to be identical
to mine as the sample is drawn randomly and should be similar to the
population proportion and not sample to sample.
...
-   **Step 4.** Calculate the bounds of the XX% confidence interval as
    the middle XX% of the bootstrap distribution.
...
95% confidence means that on average, 95% of the time the confidence
intervals would capture the population proportion.
...
The confidence interval captures the true population proportion as 62%
lies between the range of 0.533 and 0.767.
...
I would expect 95% of the intervals to capture the true population mean.
This is based on the principle that our distribution from bootstrapping
...
92% (46/50) of the confidence intervals include the population
proportion. The proportion is not exactly equal to the confidence level.
This is because we expect the proportion to be *about* 95% but it does
not have to be due to random chance of the sampling. However, if the
proportion was drastically different, such as 70%, that would be more
concerning as the proportion falls further out than the expected
proportion value.

![](images/image-1543735397.png){width="596"}
...
Choosing a confidence level of 99%, higher than our previous 95%, I
would anticipate the confidence interval to be wider. This is because as
the spread of the values from the mean become wider, so does the
interval as it is accounting for the larger values. We can verify this
running a confidence level of 99%. We see that the range for 99% is
(0.5, 0.8) as opposed to 95% (0.533, 0.767)
...
On average, we are 90 % confident the population proportion lies between
the confidence interval of (0.566, 0.75)
...
82% (41/50) of the confidence intervals include the true population
proportion. 82% is lower than the used 90% confidence level, however, it
is not drastically different from it. Because of random probability, it
is possible to get less than what would be expected on average.

![](images/image-1834794740.png)

3.  Lastly, try one more (different) confidence level. First, state how
    ...
    **infer** package and data from `samp` and interpret it. Finally,
    ...
    of intervals that capture the true population proportion.

For a 68% confidence level, I would expect the confidence interval width
to be narrower compared to previous ones.
...
The bounds of the interval is (0.583, 0.7). Running the app, we have a
proportion of 62% (31/50) that captures the true population proportion.

![](images/image-1818556881.png)

4.  Using the app, experiment with different sample sizes and comment on
    how the widths of intervals change as sample size changes (increases
    and decreases).
    
As the sample size gets larger, the smaller the intervals become. 

5.  Finally, given a sample size (say, 60), how does the width of the
...
The more bootstrap samples you take, the closer you get to reducing the standard error and closer to the population proportion
