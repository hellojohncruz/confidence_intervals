...
95% confidence means that on average, we are 95% confident the confidence intervals
would capture the population proportion.
...
92% (46/50) of the confidence intervals include the population proportion. The proportion is not exactly equal to the confidence level. This is because we expect the proportion to be *about* 95% but it does not have to be due to random chance of the sampling. However, if the proportion was drastically different, such as 80%, that would be more concerning as the proportion falls further out than the expected proportion value. 
...
Choosing a confidence level of 99%, higher than our previous 95%, I would anticipate the confidence interval to be wider. This is because as the spread of the values from the mean become wider, so does the interval as it is accounting for the larger values. We can verify this running a confidence level of 99%. We see that the range for 99% is (0.5, 0.783) as opposed to 95% (0.583, 0.717)
...
On average, we are 90 % confident the population proportion lies between the confidence interval of (0.55, 0.75)
...
4.  Lastly, try one more (different) confidence level. First, state how
...
5.  Using the app, experiment with different sample sizes and comment on
